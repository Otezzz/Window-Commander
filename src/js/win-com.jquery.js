...
(function (global, undefined) {
     ...
     * Factory method
     *
     * @throws {Error}  requires a `window` with a `document` object'
     * @param  {object} window
     */
    var factory = function (window) {
        ...
        if (typeof window.document !== 'object') {
            throw new Error('requires a `window` with a `document` object');
        }
         ...
         */
        if(!window.jQuery) {
            throw new Error('requires a `jQuery`, timo.js this plugin without jQuery');
        } else {
          $ = window.jQuery;
        }
